use std::{io, net::{SocketAddr, Ipv4Addr}, time::{Instant, SystemTime, UNIX_EPOCH}};

use crypto::{sha2::Sha256, chacha20poly1305::ChaCha20Poly1305, aead::{AeadEncryptor, AeadDecryptor}};
...
pub fn validate_token<'a>(token: &Vec<u8>, src: &SocketAddr, token_secret: &Vec<u8>) -> Option<ConnectionId<'a>> {
    let random = &token[token.len()-8..].to_vec();
    let mut tag = &mut token.clone()[token.len()-24..token.len()-8].to_vec();
    let encrypted = &token[..token.len()-24].to_vec();
    let mut secret = [0u8; 256];
    crypto::hkdf::hkdf_extract(Sha256::new(), &random, &token_secret, &mut secret);
    let mut crypto = ChaCha20Poly1305::new(&secret, &random, &[]);
    let mut decrypted = encrypted.clone();
    crypto.decrypt(&encrypted, &mut decrypted, &mut tag);
    ...
    let time: chrono::DateTime<chrono::Utc> = chrono::DateTime::from_utc(chrono::NaiveDateTime::from_timestamp_opt(i64::from_be_bytes(timestamp), 0).unwrap(), chrono::Utc);
    let duration = chrono::Utc::now().signed_duration_since(time);
    if octets == decrypted[..octets.len()] && duration.num_seconds() <= 180 {
        Some(quiche::ConnectionId::from_vec(decrypted[&octets.len()+8..].to_vec()))
    } else {
        None
    }
}