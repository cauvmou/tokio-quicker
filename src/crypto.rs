...
pub(crate) fn mint_token(dcid: &ConnectionId<'_>, src: &SocketAddr, token_secret: &[u8]) -> Vec<u8> {
...
pub(crate) fn validate_token<'a>(
    ...
    if octets == decrypted[..octets.len()] && duration.num_seconds() <= expiration_duration.unwrap_or(i64::MAX) {
...
mod test {
    ...
    fn validate_token_test() {
        let connection_id = ConnectionId::from_ref(&[6, 114, 85, 25, 219, 159, 94, 178, 209, 240, 238, 52, 117, 222, 236, 117]);
        let token: &[u8] = &[115, 35, 157, 90, 216, 123, 207, 22, 84, 203, 149, 220, 123, 81, 54, 156, 226, 47, 232, 79, 22, 177, 112, 222, 89, 251, 74, 199, 205, 192, 37, 164, 237, 24, 118, 220, 146, 175, 166, 95, 226, 187, 170, 187, 136, 44, 61, 186, 78, 4, 121, 231];
        let secret: &[u8] = &[193, 225, 35, 100, 179, 123, 28, 109, 213, 167, 40, 242, 57, 91, 85, 30];
        let socket_addr = SocketAddr::from_str("127.0.0.1:42267").unwrap();

        assert_eq!(validate_token(&token.to_vec(), &socket_addr, secret, None), Some(connection_id));
    }
    ...
    fn mint_token_test() {
        ...
        assert_eq!(validate_token(&token, &socket_addr, secret, None), Some(connection_id))
    }
}